...
public class CerrarPrueba extends JFrame {
	...
	public CerrarPrueba(int idprueba) {
		...
		buttonAceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				boolean valid = false;
				Tiempo temp1 = new Tiempo((Integer) spinnerH1.getValue(), (Integer) spinnerM1.getValue(),
						(Integer) spinnerS1.getValue(), (Integer) spinnerC1.getValue());
				Tiempo temp2 = new Tiempo((Integer) spinnerH2.getValue(), (Integer) spinnerM2.getValue(),
				...
				if (temp1.compareTo(temp2) < 0 && temp2.compareTo(temp3) < 0) {
					if (temp1.getHoras() >= 0 && temp1.getMinutos() >= 0 && temp1.getSegundos() >= 0
						...
						if (temp2.getHoras() >= 0 && temp2.getMinutos() >= 0 && temp2.getSegundos() >= 0
								&& temp2.getCentesimas() >= 0) {
							if (temp3.getHoras() >= 0 && temp3.getMinutos() >= 0 && temp3.getSegundos() >= 0
									&& temp3.getCentesimas() >= 0) {
								if (comboBoxPuesto1.getSelectedIndex() != comboBoxPuesto2.getSelectedIndex()
										&& comboBoxPuesto2.getSelectedIndex() != comboBoxPuesto3.getSelectedIndex()) {
									valid = true;
								}
							}
						}
				...
				if (valid) {
					if (chckbxDefinitivo.isSelected()) {
						...
						podio[0] = Long.parseLong(comboBoxPuesto1.getSelectedItem().toString().split("\\|")[0]);
						podio[1] = Long.parseLong(comboBoxPuesto2.getSelectedItem().toString().split("\\|")[0]);
						podio[2] = Long.parseLong(comboBoxPuesto3.getSelectedItem().toString().split("\\|")[0]);
						...
						Bronce bronce =	new Bronce(Long.parseLong(comboBoxPlatas.getSelectedItem().toString().substring(1,2)), Float.parseFloat(comboBoxBronces.getSelectedItem().toString().substring(3,7)));
						...
						String impresion = "Resultado de la prueba " + textField.getText() + " \""
								+ textFieldNombre.getText() + "\" celebrada el pasado " + spinnerFecha.getToolTipText()
								+ " en " + comboBoxLugar.getSelectedItem().toString() + "." + "\n" + "\n"
