...
public class MetalDAO implements operacionesCRUD<Metal> {
	...
	public ArrayList<Metal> buscarLibre(String tipo) {
		ArrayList<Metal> ret = new ArrayList();
		try {
			String consultaSelect = "SELECT * FROM metales WHERE asignada = 0";
			PreparedStatement pstmt = conex.prepareStatement(consultaSelect);
			ResultSet result = pstmt.executeQuery();
			while (result.next()) {
				switch(tipo) {
				case "oro":
					if(result.getLong("idoro")!=0) {
						Oro oro = new Oro(0,0);
						if(!result.getBoolean("asignada")) {
							oro.setId(result.getLong("idoro"));
							oro.setPureza(result.getFloat("pureza"));
							ret.add(oro);
						}
					}
					break;
				case "plata":
					if(result.getLong("idplata")!=0) {
						Plata pla = new Plata(0,0);
						if(!result.getBoolean("asignada")) {
							pla.setId(result.getLong("idplata"));
							pla.setPureza(result.getFloat("pureza"));
							ret.add(pla);
						}
					}
					break;
				case "bronce":
					if(result.getLong("idbronce")!=0) {
						Bronce bro = new Bronce(0,0);
						if(!result.getBoolean("asignada")) {
							bro.setId(result.getLong("idbronce"));
							bro.setPureza(result.getFloat("pureza"));
							ret.add(bro);
						}
					}
					break;
